...
export interface AgendaReady {
    ...
    // Field Lampiran Radir (Opsional agar kompatibel dengan sheet detail)
...
export default function RakordirSiapClient({ data }: RakordirSiapClientProps) {
    ...
    const handleResume = async (id: string, title: string) => {
        const isConfirmed = confirm(`Apakah Anda yakin ingin melanjutkan kembali agenda Rakordir "${title}"?`);
        if (!isConfirmed) return;

        try {
            const res = await resumeAgendaAction(id);
            if (res.success) {
                toast.success("Agenda Rakordir berhasil dilanjutkan kembali");
            } else {
