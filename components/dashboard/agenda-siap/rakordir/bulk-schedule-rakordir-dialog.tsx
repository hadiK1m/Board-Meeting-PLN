...
interface BulkScheduleRakordirDialogProps {
    selectedAgendas: AgendaReady[]
    onSuccess?: () => void
}

export function BulkScheduleRakordirDialog({ selectedAgendas, onSuccess }: BulkScheduleRakordirDialogProps) {
    ...
    // ✅ Reaktif & Tanpa Unused Var: Menggunakan useMemo menggantikan useEffect yang tidak perlu
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isAllSuccess) {
                // ✅ FIX: Parameter unused 't' dihapus sesuai warning ESLint
                toast.custom(() => (
                    <div className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-2xl border border-emerald-100 min-w-87.5 animate-in slide-in-from-bottom-5">
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            <p className="text-sm font-bold text-slate-800 leading-tight">Jadwal Ditetapkan!</p>
                            <p className="text-[11px] text-slate-500 font-medium">
                                {selectedOptions.length} Agenda Rakordir berhasil dijadwalkan.
                            </p>
                        </div>
                    </div>
                ), { duration: 4000 });
            ...
            console.error("Bulk Schedule Rakordir Error:", err)
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogTrigger asChild>
                <Button className="bg-[#125d72] hover:bg-[#05252b] text-white font-bold shadow-lg transition-all active:scale-95 h-10 px-4 rounded-xl uppercase text-[10px]">
                    <CalendarPlus className="mr-2 h-4 w-4" /> Jadwalkan ({selectedAgendas.length})
                </Button>
            </DialogTrigger>
                ...
                <div className="grid grid-cols-2 gap-5 mt-4">
                    <div className="col-span-2 space-y-2">
                        <Label className="text-[#125d72] font-bold flex justify-between items-center text-xs uppercase tracking-wider">
                            <span className="flex items-center gap-2"><Info className="h-3.5 w-3.5 text-[#14a2ba]" /> Daftar Agenda Rakordir</span>
                        </Label>
                        <Select
                            isMulti
                            isDisabled
                    ...
                    <div className="col-span-2 pt-6 flex gap-3">
                        ...
                        <Button
                            ...
                            disabled={isPending}
                            // ✅ FIX: Menggunakan flex-2 sesuai standar Tailwind canonical
                        ...
                        >
                            {isPending ? (
                                <span className="flex items-center gap-2 uppercase text-xs">
                                    <Layers className="h-4 w-4 animate-spin" /> Memproses...
                                </span>
                            ) : (
                                <span className="flex items-center gap-2 uppercase text-xs">
